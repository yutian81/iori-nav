...
window.loadGlobalCategories().catch(err => console.error('Failed to load categories:', err));
...
document.addEventListener('DOMContentLoaded', () => {
   ...
   if (closeReviewPendingModal) {
       closeReviewPendingModal.onclick = closePendingReviewModal;
   }
   if (cancelReviewPendingBtn) {
       cancelReviewPendingBtn.onclick = closePendingReviewModal;
   }
   if (reviewPendingForm) {
       reviewPendingForm.addEventListener('submit', handlePendingReviewSubmit);
   }
   if (reviewPendingModal) {
       reviewPendingModal.onclick = (e) => {
           if (e.target === reviewPendingModal) {
               closePendingReviewModal();
           }
       };
   }
